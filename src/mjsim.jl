...
struct MJSim{MN, DN, S, SE, A}
    ...
    function MJSim(m::jlModel, d::jlData; skip::Integer = DEFAULT_SKIP)
        ...
        nameshapes = map(MJSTATE_FIELDS) do name
            ...
            name => Shapes.Shape(eltype(field), size(field)...)
        end
...
function  tconstruct(::Type{MJSim}, N::Integer, m::jlModel; skip::Integer = DEFAULT_SKIP, can_share_model::Bool=DEFAULT_CAN_SHARE_MODEL)
...
function  tconstruct(::Type{MJSim}, N::Integer, modelpath::AbstractString; kwargs...)
